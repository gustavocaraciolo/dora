...
export class ProductVariantComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.productVariantService
                ...
                .subscribe(
                    (res: HttpResponse<IProductVariant[]>) => this.paginateProductVariants(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/product-variant'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    protected paginateProductVariants(data: IProductVariant[], headers: HttpHeaders) {
