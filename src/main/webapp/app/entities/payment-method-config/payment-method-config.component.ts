...
export class PaymentMethodConfigComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.paymentMethodConfigService
                ...
                .subscribe(
                    (res: HttpResponse<IPaymentMethodConfig[]>) => this.paginatePaymentMethodConfigs(res.body, res.headers),
    ...
    trackId(index: number, item: IPaymentMethodConfig) {
        return item.id;
    }

    registerChangeInPaymentMethodConfigs() {
        this.eventSubscriber = this.eventManager.subscribe('paymentMethodConfigListModification', response => this.loadAll());
    }
    ...
    protected paginatePaymentMethodConfigs(data: IPaymentMethodConfig[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.paymentMethodConfigs = data;
    }
